...
! function(a) {
	...
	b.count = 0, b.VALID_NODES = ["P", "PRE", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "H5", "H6", "DIV"], b.COLORS = ["#000000", "#444444", "#666666", "#999999", "#CCCCCC", "#EEEEEE", "#F3F3F3", "#FFFFFF", "#FF0000", "#FF9900", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#9900FF", "#FF00FF", "#F4CCCC", "#FCE5CD", "#FFF2CC", "#D9EAD3", "#D0E0E3", "#CFE2F3", "#D9D2E9", "#EAD1DC", "#EA9999", "#F9CB9C", "#FFE599", "#B6D7A8", "#A2C4C9", "#9FC5E8", "#B4A7D6", "#D5A6BD", "#E06666", "#F6B26B", "#FFD966", "#93C47D", "#76A5AF", "#6FA8DC", "#8E7CC3", "#C27BA0", "#CC0000", "#E69138", "#F1C232", "#6AA84F", "#45818E", "#3D85C6", "#674EA7", "#A64D79", "#990000", "#B45F06", "#BF9000", "#38771D", "#134F5C", "#0B5394", "#351C75", "#741B47", "#660000", "#783F04", "#7F6000", "#274E13", "#0C343D", "#073763", "#201211", "#4C1130"], b.image_commands = {
		...
		borderColor: "#252528",
		buttons: ["bold", "italic", "underline", "strikeThrough", "fontSize", "color", "sep", "formatBlock", "align", "insertOrderedList", "insertUnorderedList", "outdent", "indent", "sep", "createLink", "insertImage", "insertVideo", "undo", "redo", "html"],
		crossDomain: !1,
		direction: "ltr",
		editorClass: "",
		enableScript: !1,
		...
		return a.length && b.length ? (this.$element.removeAttr("contentEditable"), this.$element.focus(), this.setSelection(a[0], 0, b[0], 0), this.removeMarkers(), void this.$element.attr("contentEditable", !0)) : !1
		...
		}), this.$element.find("i").each(function(b, c) {
			a(c).replaceWith("<em>" + a(c).html() + "</em>")
		}), this.restoreSelectionByMarkers(), this.$textarea && this.$textarea.val(this.getHTML()); this.$element.find("span:empty").length;) this.$element.find("span:empty").remove();
		...
		var c = "Are you sure? Image will be deleted.";
		b.LANGS[this.options.language] && (c = b.LANGS[this.options.language].translation[c]), confirm(c) ? (a.remove(), this.hide(), this.saveUndoStep(), this.callback("afterRemoveImage")) : a.find("img").click()
		...
		this.$element.contents().filter(function() {
			if (this.nodeType == Node.TEXT_NODE && a(this).text().trim().length > 0 || c.indexOf(this.tagName) >= 0) b.push(this);
				...
				for (var f in b) e.append(a(b[f]).clone()), f == b.length - 1 ? a(b[f]).replaceWith(e) : a(b[f]).remove();
				b = []
			}
		});
		var e;
		...
		this.$element.get(0).onpaste = function() {
			...
			b.$element.html(""), window.setTimeout(function() {
				...
				b.$element.html(c), b.restoreSelectionByMarkers(), b.insertHTML(b.options.plainPaste ? a("<div>").html(d).text() : a.htmlClean("<div>" + d + "</div>", {
				...
				})), b.sync(), b.$element.trigger("placeholderCheck"), b.pasting = !1, b.callback("afterPaste")
			}, 1)
		}
	}, b.prototype._extractContent = function(a) {
		for (var b, c = document.createDocumentFragment(); b = a.firstChild;) c.appendChild(b);
		return c
	}, b.prototype, b.prototype.initElementStyle = function() {
		...
		var a = !1;
		this.browser.webkit && (a = 1 == this.$element.find("p").length && 1 == this.$element.find("p > br").length);
		var b = this.$element.text().replace(/(\r\n|\n|\r|\t)/gm, "");
		return ("" === b || 1 == b.length && "8203" == b.charCodeAt(0)) && 0 === this.$element.find("img").length && (0 === this.$element.find("p > br").length || a)
		...
		this.$element.on("keyup keydown focus placeholderCheck", a.proxy(function() {
			...
			if (this.isEmpty() && !this.isHTML) {
				if (this.options.paragraphy) {
					var b = a("<p>&#8203;</p>");
					this.$element.html(b), this.setSelection(b.get(0), 1, null, 1)
				}
				this.$element.addClass("f-placeholder")
			} else this.$element.find("p").length ? this.isEmpty() === !1 && this.$element.removeClass("f-placeholder") : this.options.paragraphy && (this.wrapText(), this.setSelection(this.$element.find("p")[0], this.$element.find("p").text().length, null, this.$element.find("p").text().length))
		}, this)), this.$element.trigger("placeholderCheck")
	}, b.prototype.setDimensions = function(a, b) {
		a && (this.options.height = a), b && (this.options.width = b), "auto" != this.options.height && this.$element.css("height", this.options.height), "auto" != this.options.width && this.$box.css("width", this.options.width)
		...
		var c = new Image;
		if (c.onerror = a.proxy(function() {
			this.hideImageLoader(), this.throwImageError(1)
		}, this), this.imageMode) return c.onload = a.proxy(function() {
		...
		c.onload = a.proxy(function() {
			a(d).prepend('<img alt="Image title" src="' + b + '" width="200" style="min-width: 16px; min-height: 16px; margin-bottom: ' + this.options.imageMargin + "px; margin-left: auto; margin-right: auto; margin-top: " + this.options.imageMargin + 'px">'), a(d).find("img:first").get(0).oncontrolselect = function() {
				return !1
		...
		1 == b ? c = "Bad link." : 2 == b ? c = "No link in upload response." : 3 == b ? c = "Error during file upload." : 4 == b && (c = "Parsing response failed."), this.options.imageErrorCallback && a.isFunction(this.options.imageErrorCallback) && this.options.imageErrorCallback({
			errorCode: b,
			errorStatus: c
		})
		...
		if (void 0 !== b && b.length > 0) {
			...
			if (this.drag_support.formdata && (c = this.drag_support.formdata ? new FormData : null), c && c.append(this.options.imageUploadParam, b[0]), c) {
				...
				this.options.crossDomain ? d = this.createCORSRequest("POST", this.options.imageUploadURL) : (d = new XMLHttpRequest, d.open("POST", this.options.imageUploadURL)), d.onload = a.proxy(function() {
					...
					this.throwImageError(3), this.hideImageLoader()
		...
		try {
			var c = a.parseJSON(b);
			"http://uploads.im/api" == this.options.imageUploadURL ? this.writeImage(c.data.img_url) : c.link ? this.writeImage(c.link) : this.throwImageError(2)
		} catch (d) {
			this.throwImageError(4)
		}
		...
		var e = "",
			f = "";
		if (d === !0 && (e = 'rel="nofollow"'), c === !0 && (f = 'target="_blank"'), this.imageMode) return "" !== b ? ("A" != this.$element.find(".f-img-editor").parent().get(0).tagName ? this.$element.find(".f-img-editor").wrap('<a class="f-link" href="' + b + '" ' + f + " " + e + "></a>") : (c === !0 ? this.$element.find(".f-img-editor").parent().attr("target", "_blank") : this.$element.find(".f-img-editor").parent().removeAttr("target"), d === !0 ? this.$element.find(".f-img-editor").parent().attr("rel", "nofollow") : this.$element.find(".f-img-editor").parent().removeAttr("rel"), this.$element.find(".f-img-editor").parent().attr("href", b)), this.callback("insertImageLink", [b])) : ("A" == this.$element.find(".f-img-editor").parent().get(0).tagName && a(this.$element.find(".f-img-editor").get(0)).unwrap(), this.callback("removeImageLink")), this.saveUndoStep(), this.showImageEditor(), this.$element.find(".f-img-editor").find("img").click(), this.link = !1, !1;
		...
		if (this.selectionInEditor() && "" === this.text() && "fontSize" !== a && "formatBlock" !== a && "indent" !== a && "outdent" !== a && "justifyLeft" !== a && "justifyRight" !== a && "justifyFull" !== a && "justifyCenter" !== a && "html" !== a && "undo" !== a && "redo" !== a && "selectAll" !== a && "save" !== a && "insertImage" !== a && "insertVideo" !== a && "insertOrderedList" !== a && "insertUnorderedList" !== a) return !1;
		switch (a) {
			case "fontSize":
				this.fontSize(b);
				break;
		...
		if (this.undoIndex > 1) {
			var a = this.getHTML();
			this.$element.html(this.undoStack[--this.undoIndex - 1]), this.hide(), this.callback("undo", [this.$element.html(), a])
		}
		this.refreshUndoRedo()
	}, b.prototype.redo = function() {
		if (this.undoIndex < this.undoStack.length) {
			var a = this.$element.html();
			this.$element.html(this.undoStack[this.undoIndex++]), this.hide(), this.callback("redo", [this.$element.html(), a])
		}
		this.refreshUndoRedo()
	}, b.prototype.save = function() {
		this.options.saveURL ? (this.callback("beforeSave"), a.post(this.options.saveURL, a.extend({
			body: this.getHTML()
		...
		var b;
		this.isHTML ? (this.isHTML = !1, b = a.htmlClean(this.$html_area.val(), {
			format: !0,
			removeTags: ["basefont", "center", "dir", "frame", "frameset", "isindex", "menu", "noframes", "script"]
		}), this.options.enableScript && (b = a.htmlClean(this.$html_area.val(), {
			format: !0
		})), this.$element.html(b).attr("contenteditable", !0), this.$box.removeClass("f-html"), this.$editor.find('.fr-bttn:not([data-cmd="html"])').prop("disabled", !1), this.$editor.find("div.fr-bttn").removeAttr("data-disabled"), this.$editor.find('.fr-bttn[data-cmd="html"]').removeClass("active"), this.saveUndoStep(), this.callback("htmlHide", [b])) : (this.isHTML = !0, b = this.options.inlineMode ? "\n\n" + a.htmlClean(this.$element.html(), {
			format: !0
		}) : a.htmlClean(this.$element.html(), {
			format: !0
		}), this.$html_area.val(b).trigger("resize"), this.options.inlineMode && this.$box.find(".html-switch").css("top", this.$box.css("padding-top")), this.$html_area.css("height", this.$element.height() + 20), this.$element.html(this.$html_area).removeAttr("contenteditable"), this.$box.addClass("f-html"), this.$editor.find('button.fr-bttn:not([data-cmd="html"])').prop("disabled", !0), this.$editor.find("div.fr-bttn").attr("data-disabled", !0), this.$editor.find('.fr-bttn[data-cmd="html"]').addClass("active"), this.options.inlineMode && this.hide(), this.callback("htmlShow", [b]))
		...
		for (var e in d) {
			var f = a(d[e]);
			if (c = "n" == b ? a("<div>").html(f.html()) : a("<" + b + ">").html(f.html()), f.get(0) != this.$element.get(0) && "LI" != f.get(0).tagName) {
				var g = f.prop("attributes");
				if (c.attr)
					for (var h in g) c.attr(g[h].name, g[h].value);
				f.replaceWith(c)
			} else f.html(c)
		}
		...
		if (this.clearSelection(), this.wrapText(), this.restoreSelectionByMarkers(), e === !1) {
			...
			for (var j in d) c = a(d[j]), c.get(0) != this.$element.get(0) && (i.append(a("<li>").append(c.clone())), j != d.length - 1 ? c.remove() : c.replaceWith(i));
			this.restoreSelectionByMarkers()
		}
		...
		if (b instanceof Object) this.options = a.extend({}, this.options, b), this.initOptions(), this.setCustomText(), this.setLanguage(), this.setTextNearImage();
		else {
